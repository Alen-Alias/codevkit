// MD5 implementation (pure JS, for educational purposes - not cryptographically secure)
function md5(input: string): string {
  ...
  const x = convertToWordArray(utf8Input);
  const len = utf8Input.length * 8;
...
export async function generateHashes(input: string): Promise<{
  md5: string;
  sha1: string;
  sha256: string;
  sha512: string;
